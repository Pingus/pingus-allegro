...
Pingu::Pingu()
{
  ...
  std::cout << "Pingu init ende" << std::endl;
  //id = "Normal";
}
...
Pingu::draw(BITMAP* scr)
{
  ...
  if (falling > 2) {
    ...
    if (direction == left) {
       draw_sprite(scr, sprite, x_pos - sprite->w /2 ,  y_pos- sprite->h);
    } else {
      draw_sprite_h_flip(scr, sprite, x_pos - sprite->w /2 ,  y_pos- sprite->h);
    }
  }
}
